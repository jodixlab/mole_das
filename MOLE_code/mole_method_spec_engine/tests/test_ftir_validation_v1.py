...
class FtirValidationTests(unittest.TestCase):
    ...
    def test_unreviewed_comparison_sets_hard_block_lock_and_signoff(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ftir_csv = root / "ftir.csv"
            raw_samples = root / "raw_samples.jsonl"
            ...
            self.assertFalse(bool(qa.get("lock_ready")))
            self.assertFalse(bool(qa.get("signoff_ready")))
            self.assertEqual(int(qa.get("unreviewed_comparison_set_count") or 0), 2)
            self.assertEqual((qa.get("comparison_set_review_counts") or {}).get("UNREVIEWED"), 2)
            self.assertTrue(any("explicit reviewer decision" in str(item) for item in list(qa.get("hard_blocking_issues") or [])))

    def test_explicit_set_reviews_clear_hard_blocks(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ftir_csv = root / "ftir.csv"
            raw_samples = root / "raw_samples.jsonl"
            ...
            cfg = normalize_config({
                ...
                "set_reviews": {
                    "RUN_SET_01": {
                        "decision": "ACCEPTED",
                        "reason": "alignment verified",
                        "reviewer": "peer_scientist",
                        "updated_iso": "2026-04-10T18:20:00Z",
                    },
                    "RUN_SET_02": {
                        "decision": "REJECTED",
                        "reason": "operator marked invalid",
                        "reviewer": "peer_scientist",
                        "updated_iso": "2026-04-10T18:21:00Z",
                    },
                },
            })
            payload = build_validation_package(
            ...
            qa = payload.get("qa") or {}
            self.assertEqual(list(qa.get("hard_blocking_issues") or []), [])
            self.assertEqual(int(qa.get("unreviewed_comparison_set_count") or 0), 0)
